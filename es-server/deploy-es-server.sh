...
ensure_prerequisites() {
  ...
  mkdir -p "${SCRIPT_DIR}/logs" "${SCRIPT_DIR}/logs-query"
  if command -v sudo >/dev/null 2>&1; then
    sudo chown -R 10001:10001 "${SCRIPT_DIR}/logs" "${SCRIPT_DIR}/logs-query" \
      || log "WARN: 日志目录授权失败，请手动修正10001:10001权限。"
    sudo chmod 755 "${SCRIPT_DIR}/logs" "${SCRIPT_DIR}/logs-query" \
      || log "WARN: 日志目录权限修改失败。"
  else
    chown -R 10001:10001 "${SCRIPT_DIR}/logs" "${SCRIPT_DIR}/logs-query" \
      || log "WARN: 日志目录授权失败，请手动修正10001:10001权限。"
    chmod 755 "${SCRIPT_DIR}/logs" "${SCRIPT_DIR}/logs-query" \
      || log "WARN: 日志目录权限修改失败。"
  fi
}
...
create_safety_rollback_tag() {
  ...
  ROLLBACK_TAG="rollback-${safety_version:0:80}-$(date '+%Y%m%d%H%M%S')"
