...
public class EsImportTask {
    ...
    public String importTableDateRange(String tableName, LocalDate startDate, LocalDate endDate) {
        ...
        EsImportProperties.TableConfig table = properties.requireTPlusOneTable(tableName);
    ...
    public String repairPollingDate(String tableName, LocalDate repairDate) {
        ...
        EsImportProperties.TableConfig table = properties.getPollingTables().stream()
        ...
        SyncCheckpoint checkpoint = checkpointService.find(tableName)
                .orElseThrow(() -> new ServiceException(
    ...
    private boolean executeTask(SanoImportTask task, long deadlineMillis) {
        ...
        try {
            ...
            EsImportProperties.TableConfig table = properties.getTPlusOneTables().stream()
                ...
                table = properties.getPollingTables().stream()
                        ...
                        .orElseThrow(() -> new ServiceException(
                                "ES sync task table is disabled or mode mismatch, tableName="
                                        + task.getTableName()));
                SyncCheckpoint checkpoint = checkpointService.find(task.getTableName())
                        .orElseThrow(() -> new ServiceException(
            ...
            // Polling修复从ID=0覆盖已有物理索引；TIMEOUT_PARTIAL则从持久安全断点继续
            statistics = importService.importData(config, deadlineMillis, resumeTask || pollingRepair);
