...
public class GlobalSyncMemoryLimiter {
     ...
     * 加载全局内存上限。
     */
    public GlobalSyncMemoryLimiter(EsImportProperties properties) {
        this.maxBytes = Math.max(1L,
                properties.getCommon().getWrite().getGlobalQueueMaxBytes().toBytes());
    }
        ...
        try {
            ...
            while (usedBytes + bytes > maxBytes) {
                if (stopRequested.getAsBoolean()) {
                    throw new ServiceException("ES sync stopped while waiting global memory budget");
                }
                memoryAvailable.await(STOP_CHECK_INTERVAL_SECONDS, TimeUnit.SECONDS);
            }
            if (stopRequested.getAsBoolean()) {
                throw new ServiceException("ES sync stopped before reserving global memory budget");
            }
            usedBytes += bytes;
            reservationCount++;
            ...
            throw new ServiceException("ES sync interrupted while waiting global memory budget", e);
        } finally {
    ...
    private long normalizeRequestedBytes(long requestedBytes) {
        ...
        if (bytes > maxBytes) {
            throw new ServiceException("Single ES sync batch exceeds global memory budget, requestedBytes="
    ...
    private void resize(Reservation reservation, long requestedBytes) {
        ...
        try {
            ...
            Thread.currentThread().interrupt();
            throw new ServiceException("ES sync interrupted while resizing global memory reservation", e);
        } finally {
            if (lock.isHeldByCurrentThread()) {
                lock.unlock();
            }
    ...
    public static final class Reservation implements AutoCloseable {
        ...
        private final GlobalSyncMemoryLimiter owner;
        private final AtomicBoolean closed = new AtomicBoolean(false);
        private long bytes;

        private Reservation(GlobalSyncMemoryLimiter owner, long bytes) {
            this.owner = owner;
            this.bytes = bytes;
        }
