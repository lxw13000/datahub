...
public class PollingBulkWriter {
     ...
     */
    public boolean writeBatch(SyncTableConfig tableConfig, LocalDate syncDate,
        ...
        // 每次ES整批Bulk重试前的等待时间。
        long retryIntervalMillis = Math.max(0L, properties.getPolling().getBulkRetryInterval().toMillis());
        String failureReason;
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                BulkResponse response;
                // 许可证只覆盖真实ES请求，响应判断和重试等待不占用全局并发额度
                try (GlobalEsWritePermitManager.Permit ignored = writePermitManager.acquire(TableSyncMode.POLLING)) {
                    response = client.bulk(request);
                }
                long failedCount = response.items().stream()
                        .filter(item -> item.error() != null)
                        .count();
                if (!response.errors() && failedCount == 0L && response.items().size() == rows.size()) {
                    return true;
                }
            ...
            if (attempt >= maxAttempts) {
                ...
                try {
                    ...
                    notifyService.notifyBulkFailed(tableName, indexName, syncDate,
            ...
            log.warn("===> ES-Polling bulk failed, retry whole batch. table={}, date={}, index={}, "
