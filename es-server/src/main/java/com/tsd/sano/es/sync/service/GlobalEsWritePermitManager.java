...
public class GlobalEsWritePermitManager {
    ...
    public GlobalEsWritePermitManager(EsImportProperties properties) {
        EsImportProperties.Write write = properties.getCommon().getWrite();
        this.globalConcurrency = Math.max(1, write.getGlobalBulkConcurrency());
        this.pollingReservedConcurrency = Math.max(0,
                Math.min(write.getPollingReservedConcurrency(), globalConcurrency));
        this.tPlusOneMaxConcurrency = Math.max(1,
                Math.min(write.getTPlusOneMaxConcurrency(), globalConcurrency));
    }

    /**
     * 等待并获取指定引擎的Bulk许可。
     *
     * <p>调用方必须使用try-with-resources关闭返回值。线程被中断时不吞掉中断标记，
     * 直接中止当前同步流程。</p>
     */
