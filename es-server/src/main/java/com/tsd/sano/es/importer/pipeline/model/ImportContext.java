...
public class ImportContext {
    ...
    /**
     * 每个数据批次持有的全局内存额度，批次终态或任务退出时释放。
     */
    private final Map<Long, GlobalSyncMemoryLimiter.Reservation> memoryReservations =
    ...
    public ImportBatch createBatch(List<Map<String, Object>> rows,
                                   long lastId,
                                   GlobalSyncMemoryLimiter.Reservation reservation) {
    ...
    public synchronized void completeBatch(ImportBatch batch, boolean checkpointSafe) {
        ...
        GlobalSyncMemoryLimiter.Reservation reservation = memoryReservations.remove(batch.sequence());
        if (reservation != null) {
